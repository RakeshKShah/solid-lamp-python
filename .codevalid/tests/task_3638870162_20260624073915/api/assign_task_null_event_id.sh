...
# Given
CREATE_TASK_STATUS="$(curl -sS -o "$TASK_RESP" -w '%{http_code}' -X POST "$BASE_URL/tasks" -H 'Content-Type: application/json' --data '{"title":"Task null event '"$CASE_SUFFIX"'","description":"Null event validation","status":"pending"}')"
[ "$CREATE_TASK_STATUS" = "201" ]
TASK_ID="$(jq -r '.id' "$TASK_RESP")"
[ "$TASK_ID" != "null" ]

# When
curl -sS -o "$RESP_FILE" -w '%{http_code}' -X POST "$BASE_URL/tasks/$TASK_ID/assign" -H 'Content-Type: application/json' --data '{"event_id":null}' > "$STATUS_FILE"
