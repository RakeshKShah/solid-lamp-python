...
# Given
CREATE_TASK_STATUS="$(curl -sS -o "$TASK_RESP" -w '%{http_code}' -X POST "$BASE_URL/api/tasks" -H 'Content-Type: application/json' --data '{"title":"Task with missing event '"$CASE_SUFFIX"'","description":"Event lookup failure test","status":"pending"}')"
...
# When
curl -sS -o "$RESP_FILE" -w '%{http_code}' -X POST "$BASE_URL/api/tasks/$TASK_ID/assign" -H 'Content-Type: application/json' --data '{"event_id":'"$MISSING_EVENT_ID"'}' > "$STATUS_FILE"
